...
double imf_xi(IMF *imf,double m)
{
  ...
  for(i=1;i<imf->n;i++)
    {
      z *= delta(m,imf->m[i]);
      //fprintf(stderr,"imf: %d %d %g %g   %g\n",
      //      i,imf->n,m,imf->m[i],delta(m,imf->m[i]));
    }
  return imf->k * z * y;
}
...
int imf_norm_loc_cl(IMF *imf,double m_cl,double m_max_physical)
{
  imf->k = 1;
  imf->m_cl = m_cl;
  imf->m_max_physical = m_max_physical;
  imf->m_max = imf->m_max_physical;
  /*
    imf->m_max_physical = (m_max_physical < m_cl) ?
    m_max_physical : m_cl;
...
int imf_norm_cl_pawk11(IMF *imf,double m_cl,double m_max_physical, double gauss_rnd_num, double sigma)
{
  double a,b,c;
  double mb;
  imf->k = 1;
  ...
  while(((c/b)-(a/b)) > 0.00001)
    {
      mb = imf_int_mxi(imf,imf->m[0],b)/imf_int_xi(imf,b,imf->m_max_physical)+b;
      if(mb < imf->m_cl)
	a = b;
      else
	c = b;
  ...
  imf->m_max = (pow(10,log10(b)+sigma*gauss_rnd_num)>= m_max_physical) ?
			m_max_physical : pow(10,log10(b)+sigma*gauss_rnd_num);
  fprintf(stderr,"Mmax: %f sigma :%f Mmax_applied: %f \n",b, sigma, imf->m_max);
  imf->k = (imf->m_cl-imf->m_max)  / imf_int_mxi(imf,imf->m[0],imf->m_max);
  init_lamda(imf);
  return 0;
}
// Edited code, where we want calculate Mmax given Mecl
// This will give as speed up where a lot of ssp_copies are needed

//calculate only Mmax
float imf_norm_cl_pawk11_Mmax(IMF *imf,double m_cl,double m_max_physical)
{
  double a,b,c;
  double mb;
  imf->k = 1;
...
//Normalize imf then Mmax is known
int imf_norm_cl_pawk11_fast(IMF *imf,double m_cl,double m_max_physical, float Mmax)
{
  imf->k = 1;
  imf->m_cl = m_cl;
  imf->m_max_physical = (m_max_physical < m_cl) ?
